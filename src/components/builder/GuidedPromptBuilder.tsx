      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {AUDIENCE_OPTIONS.map((opt) => (
            <Chip key={opt} label={opt} selected={audience === opt} onClick={() => setAudience(opt)} />
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {GOAL_OPTIONS.map((opt) => (
              <Chip key={opt.value} label={opt.label} selected={goal === opt.value} onClick={() => setGoal(opt.value)} />
            ))}
          </div>
        </div>
      )}
