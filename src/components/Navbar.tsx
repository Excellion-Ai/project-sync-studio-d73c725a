    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[hsl(0_0%_4%/0.85)] backdrop-blur-xl border-b border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link to="/auth" className="text-muted-foreground hover:text-foreground transition-colors text-sm font-body">
              Sign In
            </Link>
            <Link
              to="/auth"
      ...
      {mobileOpen && (
        <div className="md:hidden bg-[hsl(0_0%_4%/0.95)] backdrop-blur-xl border-b border-border px-4 pb-4 space-y-3">
          ...
          <Link to="/auth" className="block text-muted-foreground hover:text-foreground text-sm py-2 font-body">Sign In</Link>
          <Link to="/auth" className="block px-4 py-2 rounded-[10px] btn-primary text-center text-sm font-body">Start Building</Link>
        </div>
      )}
    </nav>
  );
